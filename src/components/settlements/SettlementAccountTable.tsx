            ...
            <Table>
                <TableHeader className="bg-primary rounded-xl">
                    <TableRow>
                        ...
                        <TableHead className="text-white">Sub Account</TableHead>
                        <TableHead className="text-white">Split percentage</TableHead>
                        <TableHead></TableHead>
                    </TableRow>
                </TableHeader>

                {settlementAccounts.map((item) => (
                    <TableBody onClick={() => showSettlementAccountDetails(item)} key={item.id} className="bg-white cursor-pointer">
                        <TableRow>
                            <TableCell>{item.account_number}</TableCell>
                            <TableCell>{item.bank_name}</TableCell>
                            <TableCell>{item.full_name}</TableCell>
                            <TableCell>{item.subaccount_id}</TableCell>
                            <TableCell>{`${(item.split_value * 100).toFixed(2)} %`}</TableCell>
                            <TableCell>
                                <Link className="bg-black text-white rounded-sm px-2 py-2 text-xs" href={`/settlement-accounts/${item.subaccount_id}`}>
                                    View Details
                                </Link>
                            </TableCell>
                        </TableRow>
