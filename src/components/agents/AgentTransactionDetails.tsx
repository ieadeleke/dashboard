...
>((_, ref) => {
    ...
    function handlePrintReceipt() {
        if (transaction) {
            if (transaction.NotificationDetails.ReceiptNumber) {
                window.open(transaction.NotificationDetails.ReceiptNumber, "_blank")
            } else if (transaction?.NotificationDetails?.Receipt.length) {
                window.open(transaction?.NotificationDetails?.Receipt[0], "_blank")
            } else {
                showSnackBar({
                    severity: 'error',
                    message: "An error occurred while downloading receipt"
                })
            }
        }
    }
        ...
        <Dialog open={isVisible} onOpenChange={setIsVisible}>
            {transaction && <DialogContent className="sm:max-w-[800px] max-h-[90vh] overflow-y-scroll no-scrollbar">
                ...
                <div className="grid gap-4 py-4">
                    ...
                    <DetailItem data={{
                        title: "Agency Name",
                        value: transaction?.Webguid?.length ? transaction?.Webguid[0]?.AgencyName : transaction.AgencyName
                    }} />
                    <DetailItem data={{
                        title: "Amount Paid",
                    ...
                    <DetailItem data={{
                        title: "Revenue Name",
                        value: transaction?.Webguid?.length ? transaction?.Webguid[0]?.RevName : transaction.RevName
                    }} />
                    <DetailItem data={{
                        title: "Revenue Code",
                        value: transaction?.Webguid?.length ? transaction?.Webguid[0]?.RevenueCode : transaction.RevenueCode
                    }} />
                    <DetailItem data={{
